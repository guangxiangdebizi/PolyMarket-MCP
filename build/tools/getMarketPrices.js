    ...
    async run(args) {
        try {
            ...
            if (args.market_id) {
                ...
                if (args.start_ts)
                    params.start_ts = args.start_ts;
                if (args.end_ts)
                    params.end_ts = args.end_ts;
            ...
            if (pricesData && pricesData.length > 0) {
                ...
                const prices = pricesData.map((p) => Number(p.price)).filter((p) => p > 0);
                if (prices.length > 0) {
                ...
                recentPrices.forEach((pricePoint, index) => {
                ...
                const volumes = pricesData.map((p) => Number(p.volume || 0)).filter((v) => v > 0);
                if (volumes.length > 0) {
...
//# sourceMappingURL=getMarketPrices.js.map