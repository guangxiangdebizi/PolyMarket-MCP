    ...
    async run(args) {
        try {
            ...
            const stats = {
                ...
                trades: activities.filter((a) => a.activity_type === 'TRADE').length,
                buys: activities.filter((a) => a.activity_type === 'TRADE' && a.side === 'BUY').length,
                sells: activities.filter((a) => a.activity_type === 'TRADE' && a.side === 'SELL').length,
                splits: activities.filter((a) => a.activity_type === 'SPLIT').length,
                merges: activities.filter((a) => a.activity_type === 'MERGE').length,
                redeems: activities.filter((a) => a.activity_type === 'REDEEM').length,
                rewards: activities.filter((a) => a.activity_type === 'REWARD').length,
                conversions: activities.filter((a) => a.activity_type === 'CONVERSION').length,
            ...
            if (stats.splits > 0)
                resultText += `- **Splits**: ${stats.splits}\n`;
            if (stats.merges > 0)
                resultText += `- **Merges**: ${stats.merges}\n`;
            if (stats.redeems > 0)
                resultText += `- **Redeems**: ${stats.redeems}\n`;
            if (stats.rewards > 0)
                resultText += `- **Rewards**: ${stats.rewards}\n`;
            if (stats.conversions > 0)
                resultText += `- **Conversions**: ${stats.conversions}\n`;
            ...
            if (formattedActivities.length === 0) {
                ...
                formattedActivities.forEach((activity, index) => {
                    ...
                    if (activity.outcome !== 'Unknown')
                        resultText += `- **Outcome**: ${activity.outcome}\n`;
                    ...
                    if (activity.blockNumber)
                        resultText += `- **Block**: ${activity.blockNumber}\n`;
