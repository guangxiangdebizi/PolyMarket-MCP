...
export const getOrderBook = {
  ...
  async run(args: {
    ...
    try {
      ...
      const params: any = {};
      if (args.market_id) params.market = args.market_id;
      ...
      if (includeLiquidityAnalysis) {
        const bidLiquidity = bids.reduce((sum: number, bid: any) => sum + Number(bid.size || 0), 0);
        const askLiquidity = asks.reduce((sum: number, ask: any) => sum + Number(ask.size || 0), 0);
        const totalLiquidity = bidLiquidity + askLiquidity;
        
        const bidValue = bids.reduce((sum: number, bid: any) => sum + (Number(bid.size || 0) * Number(bid.price || 0)), 0);
        const askValue = asks.reduce((sum: number, ask: any) => sum + (Number(ask.size || 0) * Number(ask.price || 0)), 0);
      ...
      if (asks.length > 0) {
        ...
        asks.slice().reverse().forEach((ask: any) => {
