...
export const getMarkets = {
  ...
  async run(args: {
    ...
    try {
      ...
      if (args.liquidity_min) params.liquidity_num_min = args.liquidity_min;
      if (args.volume_min) params.volume_num_min = args.volume_min;
      ...
      if (formattedMarkets.length === 0) {
        ...
        formattedMarkets.forEach((market: any, index: number) => {
          ...
          if (market.startDate) resultText += `- **Start Date**: ${new Date(market.startDate).toLocaleDateString()}\n`;
          if (market.endDate) resultText += `- **End Date**: ${new Date(market.endDate).toLocaleDateString()}\n`;
          if (market.description) resultText += `- **Description**: ${market.description}\n`;
          if (market.outcomes && market.outcomes.length > 0) {
            resultText += `- **Outcomes**: ${market.outcomes.map((o: any) => o.title || o).join(', ')}\n`;
          }
          if (market.tags && market.tags.length > 0) {
            resultText += `- **Tags**: ${market.tags.map((t: any) => t.label || t).join(', ')}\n`;
          }
