...
export const getEvents = {
  ...
  async run(args: {
    ...
    try {
      ...
      if (args.liquidity_min) params.liquidity_min = args.liquidity_min;
      if (args.volume_min) params.volume_min = args.volume_min;
      ...
      if (formattedEvents.length === 0) {
        ...
        formattedEvents.forEach((event: any, index: number) => {
          ...
          if (event.startDate) resultText += `- **Start Date**: ${new Date(event.startDate).toLocaleDateString()}\n`;
          if (event.endDate) resultText += `- **End Date**: ${new Date(event.endDate).toLocaleDateString()}\n`;
          if (event.description) resultText += `- **Description**: ${event.description}\n`;
          if (event.tags && event.tags.length > 0) {
            resultText += `- **Tags**: ${event.tags.map((t: any) => t.label || t).join(', ')}\n`;
          }
          
          // Show markets within the event
          if (event.markets && event.markets.length > 0) {
            resultText += `\n### Markets in this Event:\n`;
            event.markets.slice(0, 5).forEach((market: any, marketIndex: number) => {
              resultText += `${marketIndex + 1}. **${market.question || market.title}**\n`;
              if (market.volume) resultText += `   - Volume: $${Number(market.volume).toLocaleString()}\n`;
              if (market.outcomes) resultText += `   - Outcomes: ${market.outcomes.map((o: any) => o.title || o).join(', ')}\n`;
            });
