...
export const getMarketHolders = {
  ...
  async run(args: {
    ...
    try {
      ...
      if (args.min_balance) params.min_balance = args.min_balance;
      ...
      const totalShares = holders.reduce((sum: number, holder: any) => sum + Number(holder.balance || 0), 0);
      const avgHolding = totalHolders > 0 ? totalShares / totalHolders : 0;
      
      // Calculate concentration metrics
      const sortedHolders = [...holders].sort((a: any, b: any) => Number(b.balance || 0) - Number(a.balance || 0));
      const top10Holdings = sortedHolders.slice(0, 10).reduce((sum: number, holder: any) => sum + Number(holder.balance || 0), 0);
      ...
      if (formattedHolders.length === 0) {
        ...
        formattedHolders.forEach((holder: any, index: number) => {
          ...
          if (includeUserInfo && holder.alias) {
            ...
            if (holder.bio) resultText += `- **Bio**: ${holder.bio}\n`;
            if (holder.isVerified) resultText += `- **Status**: ✅ Verified\n`;
          } else {
        ...
        if (formattedHolders.length >= 5) {
          ...
          const whales = formattedHolders.filter((h: any) => h.percentage > 5);
          const whaleShares = whales.reduce((sum: number, h: any) => sum + h.balance, 0);
          ...
          const largeHolders = formattedHolders.filter((h: any) => h.percentage > 1 && h.percentage <= 5);
          const largeHolderShares = largeHolders.reduce((sum: number, h: any) => sum + h.balance, 0);
          ...
          const mediumHolders = formattedHolders.filter((h: any) => h.percentage > 0.1 && h.percentage <= 1);
          const mediumHolderShares = mediumHolders.reduce((sum: number, h: any) => sum + h.balance, 0);
          ...
          const smallHolders = formattedHolders.filter((h: any) => h.percentage <= 0.1);
          const smallHolderShares = smallHolders.reduce((sum: number, h: any) => sum + h.balance, 0);
