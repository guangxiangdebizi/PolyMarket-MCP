...
export const getTrades = {
  ...
  async run(args: {
    ...
    try {
      ...
      if (args.user_address) params.user_address = args.user_address;
      if (args.side) params.side = args.side;
      if (args.min_size) params.min_size = args.min_size;
      if (args.max_size) params.max_size = args.max_size;
      ...
      const stats = {
        ...
        buyTrades: trades.filter((t: any) => t.side === 'BUY').length,
        sellTrades: trades.filter((t: any) => t.side === 'SELL').length,
        totalVolume: trades.reduce((sum: number, t: any) => sum + Number(t.size || 0), 0),
        totalValue: trades.reduce((sum: number, t: any) => sum + (Number(t.size || 0) * Number(t.price || 0)), 0),
        avgPrice: 0,
        avgSize: 0,
        uniqueMarkets: new Set(trades.map((t: any) => t.market_id)).size,
        uniqueUsers: new Set(trades.map((t: any) => t.user_address)).size
      };

      if (trades.length > 0) {
        stats.avgPrice = trades.reduce((sum: number, t: any) => sum + Number(t.price || 0), 0) / trades.length;
      ...
      if (args.market_id) filterDescription += ` for market ${args.market_id}`;
      if (args.user_address) filterDescription += ` by user ${args.user_address.slice(0, 6)}...${args.user_address.slice(-4)}`;
      if (args.side) filterDescription += ` (${args.side} only)`;
      ...
      if (formattedTrades.length === 0) {
        ...
        formattedTrades.forEach((trade: any, index: number) => {
          ...
          if (trade.blockNumber) resultText += `- **Block**: ${trade.blockNumber}\n`;
